...
## 8. Fix: split unit notation from URL-safe slug

Post-implementation review requested three fixes to `Unit`, addressing the `{slug:path}`/case-insensitive-slug workarounds flagged as fragile in the original design.

- [x] 8.1 Add `unit` column to `Unit` (`String(16)`, not-null) carrying the physical unit notation (e.g. `"mg/L"`, `"pH"`) — the value the measurement API actually exchanges. `slug` becomes a derived, URL-safe routing key: lowercase, `/` replaced with `_` (e.g. `"mg_l"`), via new `src/unit_slug.py::slugify_unit()`.
- [x] 8.2 New migration `20260731_000005_units_add_unit_and_normalize_slug.py`: adds `unit` (backfilled from the current `slug` values), normalizes existing `slug` values in place (`lower(replace(slug, '/', '_'))`), replaces the functional case-insensitive slug index with a plain unique index. `parameter_units` rows are unaffected (FK by `unit_id`, not by slug string). Verified end-to-end against a real, isolated Postgres instance (upgrade, downgrade, re-upgrade), same procedure as 2.6.
- [x] 8.3 `UnitRepository.create()` now takes `unit` (not `slug`) and derives `slug` internally; added `get_by_unit(unit)` (case-insensitive match on the notation column) alongside the existing `get_by_slug(slug)` (exact match on the already-lowercase routing key) — two lookup methods for two different callers (URL routing vs. measurement-notation matching).
- [x] 8.4 `src/units.py`: `CreateUnitRequest` now takes `unit` instead of `slug`; `UnitPayload` returns both `slug` and `unit`; `UpdateUnitRequest` unchanged (neither field editable). Routes revert from `{slug:path}` to a plain `{slug}` — no longer needed since `slug` can't contain `/`.
- [x] 8.5 `src/aquarium_measurements.py`/`src/aquarium_measurement_repository.py`: measurement unit validation/canonicalization/response-serialization switched from `get_by_slug`/`.unit.slug` to `get_by_unit`/`.unit.unit`, preserving the existing measurement API contract (notation strings, not the new URL-safe slug).
- [x] 8.6 Updated `src/db.py` seeding (`_SEED_UNITS`/`_seed_units`/`_seed_parameter_units`) and all affected tests (`tests/test_unit_repository.py`, `tests/test_units.py`, `tests/test_aquarium_measurement_repository.py`, `tests/test_parameter_repository.py`) for the new `unit`/`slug` split. Full suite: 164 passed. `ruff`/`mypy` clean.
- [x] 8.7 Updated `proposal.md`, `design.md`, and both delta specs to reflect the revised design (documented as a revision, not silently rewritten).
