...
class TestValidateToken:
    ...
    async def test_validate_token_with_valid_token(
        self, create_valid_token, auth_settings, mock_jwks, mock_oidc_config
    ):
    ...
    async def test_validate_token_with_expired_token(
        self, create_expired_token, auth_settings, mock_jwks
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_token_with_invalid_issuer(
        self, create_valid_token, auth_settings, mock_jwks, mock_rsa_keys
    ):
        """Test that tokens with wrong issuer are rejected."""
        token = create_valid_token(
            sub="user123", aud="test-client-id", issuer="https://wrong-issuer.com"
        )
    ...
    @pytest.mark.asyncio
    async def test_validate_token_with_invalid_audience(
        self, create_valid_token, auth_settings, mock_jwks
    ):
...
class TestGetCurrentUser:
    ...
    @pytest.mark.asyncio
    async def test_get_current_user_with_valid_credentials_via_endpoint(
        self, create_valid_token, auth_settings, mock_jwks
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_current_user_with_invalid_credentials_via_endpoint(
        self, auth_settings, mock_jwks
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_current_user_with_expired_token_via_endpoint(
        self, create_expired_token, auth_settings, mock_jwks
    ):
